...
for (const ctx of mains) {
  ...
  if (typeof out === "string" && out.startsWith("{\"capturedAt")) { fs.writeFileSync(process.env.OUT, JSON.stringify(JSON.parse(out), null, 1)); console.log("saved", process.env.OUT, fs.statSync(process.env.OUT).size) } else console.log(out)
}
ws.close()
